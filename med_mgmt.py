...
def make_bill(): # makes bill
    ...
    m+="===============================================\n"
    m+="                                  No :%s\n\n" % det[5]
    m+="          SRM CHEMIST AND DRUGGIST\n"
    m+="  SRM Institute of Science and Technology, Kattankulathur\n\n"
    m+="-----------------------------------------------\n"
    if t==1:
        m+="Name: %s\n" % named
        m+="Address: %s\n" % addd
...
def again():    #for login window-----------------------------------------------------------------------------LOGIN WINDOW
    ...
    if flag=='apt':
        apt.destroy()
    root=Tk()
    root.title('SRM SOFTWARE SOLUTIONS')
    Label(root,text='CHEMIST AND DRUG SHOP').grid(row=0,column=0,columnspan=5)
    Label(root,text="SRM Institute of Science and Technology, Kattankulathur").grid(row=1,column=0,columnspan=5)
...
def open_cus(): #OPENS MAIN MENU----------------------------------------------------------------------------MAIN MENU
    ...
    apt=Tk()
    apt.title("Interface")
    Label(apt, text="SRM CHEMIST AND DRUGGIST").grid(row=0,column=0)
